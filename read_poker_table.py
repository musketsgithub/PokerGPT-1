...
class ReadPokerTable:
    ...
    def detect_text_changed(self, player_number, unique_id, relative_x, relative_y, width, height, typeofthing=None):
        ...
        if self.has_image_changed(unique_id, screenshot_array):
            ...
            if typeofthing=='action':
                plt.imshow(screenshot_array)
                plt.title(detected_text.strip())
                plt.show()
    ...
    def detect_player_stack_and_action(self, player_number):
        ...
        detected_stack_text = self.detect_text_changed(player_number, player_number + 10, region_stack_x, region_stack_y, width, height, typeofthing='stack')
        ...
        print(f"Stack text: {detected_stack_text}")
        ...
        detected_action_text = self.detect_text_changed(player_number, player_number + 20, region_action_x,
                                                        region_action_y, width, height, typeofthing='action')
    ...
    def find_cards_on_table(self):
        card_number_positions = [
            (0.355, 0.382),  # Card 1 position coordinates x,y
            (0.421, 0.382),  # Card 2 position coordinates x,y
            (0.486, 0.382),  # Card 3 position coordinates x,y
            (0.553, 0.382),  # Card 4 position coordinates x,y
            (0.620, 0.382)  # Card 5 position coordinates x,y
        ]

        card_icon_positions = [
            (0.339, 0.421),  # Card 1 position coordinates x,y
            (0.403, 0.421),  # Card 2 position coordinates x,y
            (0.470, 0.421),  # Card 3 position coordinates x,y
            (0.536, 0.421),  # Card 4 position coordinates x,y
            (0.602, 0.421)  # Card 5 position coordinates x,y
        ]
        ...
        for index, (icon_position, number_position) in enumerate(zip(card_icon_positions, card_number_positions),
            ...
            icon_width, icon_height = 10, 7  # Card icon dimensions
            num_width, num_height = 16, 24  # Card number/letter dimensions

            # Capture and process card suit
            screenshot_icon = self.capture_screen_area(x, y, icon_width, icon_height, resize_dimensions=(10,7))
            ...
            # Capture and process card number/letter using template matching
            screenshot_num = self.capture_screen_area(num_x, num_y, num_width, num_height, resize_dimensions=(12,21))
            screenshot_num_gray = cv2.cvtColor(np.array(screenshot_num), cv2.COLOR_BGR2GRAY)
    ...
    def find_dealer_button(self, button_template):
        ...
        dealer_button_regions = {
            1: (0.391, 0.632),  # done - these motherfuckers change positions randomly
            2: (0.192, 0.490),  # done - these motherfuckers change positions randomly
            3: (0.208, 0.372),  # done - these motherfuckers change positions randomly
            4: (0.528, 0.290),  # done - these motherfuckers change positions randomly
            5: (0.776, 0.366),  # done - these motherfuckers change positions randomly
            6: (0.724, 0.582)  # done - these motherfuckers change positions randomly
        }

        for player_number, (region_x, region_y) in dealer_button_regions.items():

            width, height = 15,12 # Adjust these dimensions as necessary
